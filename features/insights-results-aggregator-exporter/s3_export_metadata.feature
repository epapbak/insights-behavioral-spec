...
Feature: Ability to export metadata into S3
  ...
  Scenario: Check export metadata table into S3 object
     ...
     When I run the exporter with the following command line flags: --metadata --output S3
     Then The process should finish with exit code 0
      ...
      And I should see following number of records stored in CSV objects in S3
          ...
          | rule_hit.csv                           | 0       |
          | _metadata.csv                          | 11      |
      And I should see following records in exported object _metadata.csv placed in columns 0 and 1
          | Table name                             | Records |
          | advisor_ratings                        | 0       |
          | report                                 | 0       |
  ...
  Scenario: Check export metadata table into S3 object when REPORT table is not empty
     ...
     When I insert following records into REPORT table
      ...
      And I run the exporter with the following command line flags: --metadata --output S3
     Then The process should finish with exit code 0
      And I should see following objects generated in S3
          | File name                              |
          | advisor_ratings.csv                    |
