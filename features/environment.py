...
def clean_db(context):
    connection_string = "host={} port={} dbname={} user={} password={}".format(
        context.database_host, context.database_port, "test",
        context.database_user, context.database_password,
    )
...
def create_notification_db(context):
    ...
    try:
        ...
        if err.pgerror != 'ERROR: database "notification" already exists' and err.pgcode != '42P04':
...
def setup_default_S3_context(context):
    ...
    context.S3_secret_access_key = os.getenv("S3_SECRET_ACCESS_KEY", "test_secret_access_key")
